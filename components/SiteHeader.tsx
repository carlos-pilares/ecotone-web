...
type SiteHeaderProps = {
  /**
   * Experience (and similar) pages: the bar sits over a light page background from first paint, not
   * a full-bleed dark hero. SSR must include `solid` and the CMS logo must start in “dark” mode
   * so we don’t flash light-on-light until `useEffect` runs.
   */
  mainNavSolid?: boolean
}

/**
 * Site-wide primary navigation (same markup as the home page).
 * Single source of truth for the top menu across routes.
 * CMS: `siteSettings` (`header` + logos); same fallbacks as `data/cmsApproved/siteSettingsApprovedContent` if missing.
 */
export async function SiteHeader({ mainNavSolid = false }: SiteHeaderProps = {}) {
  const shell = await getSiteSettingsShell()
  const { headerLogoLightUrl, headerLogoDarkUrl, homePath, mainNav, primaryCta, mobileMenuAriaLabel } = shell
    ...
    <>
      <nav className={navClassName} id="topNav">
        <div className="nav-inner">
          ...
          <button type="button" className="hamburger" id="ham" aria-label={mobileMenuAriaLabel}>
            <span />
            <span />
            <span />
          </button>

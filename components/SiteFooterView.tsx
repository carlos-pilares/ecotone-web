...
import { FooterMobileAccordions } from '@/components/FooterMobileAccordions'
import {
  splitFooterColumnLinks,
  type FooterCredentialResolved,
      ...
      <a
        ...
        onClick={
          isWhatsappHref(item.href)
            ? () => trackWhatsappClick({ button_location: 'footer', cta_id: CTA_IDS.FOOTER_WHATSAPP })
            : undefined
        }
      >
        {item.label}
...
function FooterLinksColumnDesktop({
  column,
  colKey,
}: {
  ...
  return (
    <div className="foot-col foot-col--desktop">
...
function FooterCredential({ item }: { item: FooterCredentialResolved }) {
  if (item.href) {
    return (
      <a
        className="foot-cert"
  ...
  return <span className="foot-cert">{item.label}</span>
}

function FooterSocialIcons({ items }: { items: FooterSocialItemResolved[] }) {
          ...
          <li key={`social-${i}`}>
            {item.href ? (
              <a
                ...
                aria-label={item.alt}
              >
                <img src={item.iconUrl} alt="" className="foot-social-icon" decoding="async" />
              </a>
            ) : (
              <span className="foot-social-item" aria-label={item.alt}>
                <img src={item.iconUrl} alt="" className="foot-social-icon" decoding="async" />
              </span>
            )}
          </li>
    ...
    <footer className="footer">
      ...
      <div className="footer-inner">
        <div className="foot-brand-col">
          ...
          {brand.tagline ? <div className="foot-tagline">{brand.tagline}</div> : null}
          {(brand.descriptionLine1 || brand.descriptionLine2) && (
            <p className="foot-brand-desc">
              {brand.descriptionLine1 ? (
                <>
                  {brand.descriptionLine1}
                  {brand.descriptionLine2 ? <br /> : null}
                </>
              ) : null}
      ...
      <div className="foot-bottom">
        ...
        {bottomBar.credentials.length > 0 ? (
          <div className="foot-certs">
            {bottomBar.credentials.map((item, i) => (
              <Fragment key={`cert-${i}`}>
                {i > 0 ? <span className="foot-cert-sep" style={decDivider} aria-hidden /> : null}
                <FooterCredential item={item} />
              </Fragment>
            ))}
          </div>
        ) : null}

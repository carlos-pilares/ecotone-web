    ...
    <section className="content-section bg-warm" id="book">
      <div className="content-inner">
        ...
        <div className="booking-card">
          ...
          {data.wetravelUrl.trim() && data.wetravelLabel.trim() ? (
            <a className="btn-book-main" href={data.wetravelUrl} target="_blank" rel="noopener noreferrer">
              {data.wetravelLabel}
            </a>
          ) : null}
          {data.whatsappUrl.trim() && data.whatsappLabel.trim() ? (
          ...
          <div className="trust-strip">
            {data.trustStripItems.map((item, i) => {
              const mod = i % 3
              return (
          ...
          <p className="book-terms-note">
            {data.termsNote}
            {data.termsHash.trim() && data.termsLinkLabel.trim() ? (
              <>
                {' '}
                <a href={data.termsHash}>{data.termsLinkLabel}</a>.
              </>
            ) : (
              '.'
            )}

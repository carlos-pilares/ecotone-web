...
const LOG = '[api/enquiry/update-travel-date]'
...
export async function POST(request: Request) {
  ...
  if (!travelDate || !ALLOWED_FIXED_TRAVEL_DATES.has(travelDate)) {
  ...
  try {
    ...
    const result = await updateNormalizedLeadTravelDate(
      sheets,
      spreadsheetId,
      sheetTab,
      leadId,
    ...
    console.error(`${LOG} ok=false reason=unhandled message=${message}`)

/**
 * SAFE attribute cleanup — apply the approved 31-path unset plan.
 *
 * Reads the exact plan from:
 *   backups/sanity-safe-cleanup-before.json
 *
 ...
 *
 * Default: DRY RUN (lists matched/mismatched; no writes).
 * Write:   --commit
 *
 * Usage:
 *   npx tsx scripts/cleanupSanitySafeAttributesApply.ts
 *   npx tsx scripts/cleanupSanitySafeAttributesApply.ts --commit
 *
 * Does NOT change schemas. Patches only `unset` of matched paths.
 */
...
const SNAPSHOT_PATH = resolve(process.cwd(), 'backups/sanity-safe-cleanup-before.json')
...
/** Stable stringify so object key order does not cause false mismatches. */
...
async function main() {
  ...
  if (snapshot.entries.length !== 31) {
    console.warn(
      `Warning: expected 31 SAFE paths, found ${snapshot.entries.length} in snapshot JSON`,
    )
  }

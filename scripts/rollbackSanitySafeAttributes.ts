/**
 * Rollback SAFE attribute cleanup using the field snapshot JSON.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/rollbackSanitySafeAttributes.ts           # dry-run
 *   npx tsx scripts/rollbackSanitySafeAttributes.ts --commit  # write
 *
 * Requires .env.local write token (SANITY_API_TOKEN). Scripts strip a known
 * TU_TOKEN_AQUI placeholder fragment if present; do not commit secrets.
 */
